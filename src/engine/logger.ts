...
export const log = {
  ...
  file: (filename: string, message: string) => {
    const tag = `${COLORS.blue}${filename}${COLORS.reset}`;
    console.info(`${COLORS.dim}${timestamp()}${COLORS.reset}  ${tag}  ${message}`);
  },

  fileProgress: (filename: string, stats: { scanned: number; inserted: number; skipped: number; errors: number }) => {
    const tag = `${COLORS.blue}${filename}${COLORS.reset}`;
    const parts: string[] = [
  ...
  migration: {
    start: (profileName: string, config: { batchSize: number; maxId?: number; fileCount: number; resuming: boolean }) => {
      const resuming = config.resuming ? `${COLORS.yellow}yes${COLORS.reset}` : 'no';
      const lines = [
        '',
        `${COLORS.bold}Migration started [${profileName}]${COLORS.reset}`,
        `  batch size:     ${formatNumber(config.batchSize)}`,
        `  max id:         ${config.maxId ? formatNumber(config.maxId) : 'none'}`,
        `  files to process: ${config.fileCount}`,
        `  resuming:       ${resuming}`,
        '',
      ];
    ...
    summary: (stats: {
      ...
      const lines = [
        ...
        '',
        `  files:    ${formatNumber(stats.filesProcessed)} / ${formatNumber(stats.filesTotal)}`,
        `  scanned:  ${COLORS.bold}${formatNumber(stats.scanned)}${COLORS.reset}`,
        `  inserted: ${COLORS.green}${formatNumber(stats.inserted)}${COLORS.reset}`,
